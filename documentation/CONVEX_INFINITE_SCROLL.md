...
export const setup = () => {
    if (env.PUBLIC_CONVEX_URL) {
        setupConvex(env.PUBLIC_CONVEX_URL, { unsavedChangesWarning: false })
    }
}
...
<script lang="ts">
    ...
    type Item = { _id: string; _creationTime: number; name: string }
    ...
    async function loadMore() {
        ...
        const result = await client.query(api.items.listPaginated, {
            cursor,
            limit: 50
        })
    ...
    // Live indicator
    const isLive = $derived(!liveQuery.isLoading && !liveQuery.error)
</script>
...
<VirtualList items={items()} defaultEstimatedItemHeight={60} onLoadMore={loadMore} {hasMore}>
...
- [VirtualList Infinite Scroll API](../README.md#infinite-scroll)
